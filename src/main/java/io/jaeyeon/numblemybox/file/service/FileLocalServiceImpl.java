...
public class FileLocalServiceImpl implements FileService {
  ...
  @Override
  public UploadFileResponse upload(MultipartFile file, Long folderId, String rootFolderName, Member owner) throws IOException {
  ...
  private Folder findOrCreateRootFolder(Member owner, String rootFolderName) {
    return folderRepository.findByNameAndOwner(rootFolderName, owner)
            .orElseThrow(() -> {
  ...
  public Resource downloadFile(String encodedFileName, Member owner) throws IOException {
    ...
    FileEntity fileEntity = filesRepository.findByFileName(fileName)
